...
이번 섹션에서는 앞서 배운 개념들을 PyTorch로 구현하고 실제로 학습, 추론, 평가하는 방법을 학습합니다.

## 8.1. 학습 (Training)

**Copy Task를 통한 Transformer 학습 예제**

Copy Task는 입력 시퀀스를 그대로 출력하는 간단한 작업입니다. Transformer가 제대로 작동하는지 검증하기에 좋은 테스트입니다.
...
def data_gen(V, batch, nbatches):
    ...
    반환값:
        배치 데이터를 생성하는 generator
    """
    for i in range(nbatches):
        # 1부터 V-1까지의 랜덤 정수 생성 (0은 padding으로 예약)
        data = torch.from_numpy(np.random.randint(1, V, size=(batch, 10)))
        data[:, 0] = 1  # 시작 토큰을 1로 설정

        # 소스와 타겟을 동일하게 설정 (Copy Task)
...
class SimpleLossCompute:
    """
    간단한 Loss 계산 및 학습 클래스
    """
    def __init__(self, generator, criterion, opt=None):
        """
        매개변수:
            generator: 모델의 출력을 vocabulary 확률로 변환하는 레이어
            criterion: Loss 함수 (예: LabelSmoothing)
            opt: Optimizer (None이면 평가 모드)
        """
    ...
    def __call__(self, x, y, norm):
        ...
        매개변수:
            x: 모델 출력 (batch_size, seq_len, d_model)
            y: 정답 레이블 (batch_size, seq_len)
            norm: 정규화 상수 (일반적으로 토큰 수)

        반환값:
            정규화된 loss 값
        """
        # Generator를 통해 vocabulary 확률 계산
        x = self.generator(x)
...
def run_epoch(data_iter, model, loss_compute):
    ...
    매개변수:
        data_iter: 데이터 iterator
        model: Transformer 모델
        loss_compute: SimpleLossCompute 인스턴스
...
def train_model():
    ...
    # 모델 생성
    model = make_model(V, V, N=2)  # 2개 레이어로 간단하게
    ...
    for epoch in range(10):
        ...
        train_loss = run_epoch(
            data_gen(V, 30, 20),  # 30개 배치, 20번 반복
            model,
...
def greedy_decode(model, src, src_mask, max_len, start_symbol):
    ...
    매개변수:
        ...
        src: 입력 시퀀스 (1, src_len)
        src_mask: 입력 마스크
        max_len: 생성할 최대 길이
        start_symbol: 시작 토큰

    반환값:
        생성된 시퀀스 (1, output_len)

    동작 방식:
        1. Encoder로 입력을 인코딩
        2. 시작 토큰부터 시작
        3. 매 스텝마다 가장 확률이 높은 토큰 선택
        4. 선택된 토큰을 다음 입력으로 사용
        5. max_len에 도달할 때까지 반복
    """
    # Encoder를 통과
...
def beam_search_decode(model, src, src_mask, max_len, start_symbol,
    ...
    Greedy와의 차이:
        - Greedy: 매 스텝마다 1개의 최고 확률 토큰만 선택
        - Beam Search: beam_size개의 후보를 동시에 추적
        - 더 나은 전역 최적해를 찾을 가능성이 높음
    """
    memory = model.encode(src, src_mask)
    ...
    for i in range(max_len - 1):
        ...
        for seq, score in beams:
            # 이미 종료된 시퀀스는 그대로 유지
            if seq[0, -1].item() == 2:  # 2 = EOS 토큰
                candidates.append((seq, score))
                continue

            # Decoder 통과
            out = model.decode(memory, src_mask, seq,
                             subsequent_mask(seq.size(1)).type_as(src.data))
            prob = model.generator(out[:, -1])
            log_prob = torch.log_softmax(prob, dim=-1)

            # 상위 beam_size개의 토큰 선택
            topk_log_probs, topk_indices = torch.topk(log_prob, beam_size)

            for k in range(beam_size):
                next_token = topk_indices[0, k].item()
                next_log_prob = topk_log_probs[0, k].item()
                new_seq = torch.cat([seq,
                                    torch.ones(1, 1).type_as(src.data).fill_(next_token)],
                                   dim=1)

                # 길이 정규화 적용
                new_score = score + next_log_prob
                length_penalty = ((5 + new_seq.size(1)) / 6) ** alpha
...
def inference_example(model):
    ...
    print("추론 예제")
    print("=" * 50)
    print(f"입력 시퀀스: {src.data.tolist()[0]}")

    # Greedy Decoding
    print("\n[Greedy Decoding]")
    greedy_result = greedy_decode(model, src, src_mask,
                                  max_len=10, start_symbol=1)
    print(f"출력 시퀀스: {greedy_result.data.tolist()[0]}")

    # Beam Search Decoding
    print("\n[Beam Search Decoding (beam_size=5)]")
    beam_result = beam_search_decode(model, src, src_mask,
                                     max_len=10, start_symbol=1,
                                     beam_size=5)
    print(f"출력 시퀀스: {beam_result.data.tolist()[0]}")

    print("\n✅ Copy Task 성공 여부:")
    input_seq = src.data.tolist()[0]
    greedy_seq = greedy_result.data.tolist()[0]
    print(f"입력 == Greedy 출력: {input_seq == greedy_seq}")

# 사용 예시
# inference_example(trained_model)
```
...
def compute_perplexity(model, data_iter, criterion):
    """
    Perplexity 계산
    ...
    with torch.no_grad():
        for batch in data_iter:
            ...
            # Loss 계산
            pred = model.generator(out)
            loss = criterion(pred.contiguous().view(-1, pred.size(-1)),
                           batch.trg_y.contiguous().view(-1))

            total_loss += loss.item()
            total_tokens += batch.ntokens

    # Perplexity = exp(평균 loss)
...
def compute_bleu(references, hypotheses, max_n=4):
    ...
    매개변수:
        references: 정답 시퀀스 리스트
        hypotheses: 예측 시퀀스 리스트
        max_n: 최대 n-gram 크기
    ...
    def modified_precision(ref, hyp, n):
        ...
        overlap = sum(min(hyp_ngrams[ng], ref_ngrams[ng])
    ...
    for n in range(1, max_n + 1):
        prec_sum = sum(modified_precision(ref, hyp, n)
                      for ref, hyp in zip(references, hypotheses))
        precisions.append(prec_sum / len(references))
    ...
    # BLEU 계산 (균일한 가중치)
    log_precisions = [np.log(p) if p > 0 else float('-inf')
                      for p in precisions]

    if any(p == float('-inf') for p in log_precisions):
        return 0.0

    bleu = bp * np.exp(sum(log_precisions) / max_n)
    return bleu
...
def evaluate_model(model, test_data_iter, criterion):
    """
    모델의 종합 성능 평가

    매개변수:
        model: 평가할 모델
        test_data_iter: 테스트 데이터
        criterion: Loss 함수
    ...
    # 1. Accuracy
    accuracy = compute_accuracy(model, test_data_iter)
    print(f"✅ Accuracy: {accuracy:.4f} ({accuracy*100:.2f}%)")

    # 2. Perplexity
    perplexity = compute_perplexity(model, test_data_iter, criterion)
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_data_iter):
            ...
            # 예측 생성
            src = batch.src[0:1]  # 첫 번째 샘플만
            src_mask = (src != 0).unsqueeze(-2)

            pred = greedy_decode(model, src, src_mask,
                                max_len=10, start_symbol=1)

            # 정답과 예측 비교
            ref_tokens = batch.trg[0].tolist()
            ...
            print(f"샘플 {i+1}:")
            print(f"  입력: {src[0].tolist()}")
            print(f"  정답: {ref_tokens}")
            print(f"  예측: {pred_tokens}")
            print(f"  일치: {'✅' if ref_tokens == pred_tokens else '❌'}")
...
def run_evaluation():
    ...
    # model.load_state_dict(torch.load('model.pt'))  # 저장된 모델 로드

    # 평가 데이터 생성
    test_data = list(data_gen(V, 30, 10))

    # Loss 함수
    criterion = LabelSmoothing(size=V, padding_idx=0, smoothing=0.0)

    # 평가 수행
    results = evaluate_model(model, test_data, criterion)
...
def full_pipeline():
    ...
    V = 11
    print("=" * 60)
    print("Transformer Copy Task - 전체 파이프라인")
    print("=" * 60)
    print(f"어휘 크기: {V}")
    print(f"모델 구조: N=2 layers, h=8 heads, d_model=512")
    ...
    print("최종 결과 요약")
    print("=" * 60)
    print(f"✅ 학습 완료: {10} epochs")
    print(f"✅ 최고 검증 Loss: {best_val_loss:.4f}")
    print(f"✅ 테스트 Accuracy: {results['accuracy']:.4f}")
    print(f"✅ 테스트 Perplexity: {results['perplexity']:.4f}")
    print(f"✅ 테스트 BLEU: {results['bleu']:.4f}")
    print("\n🎉 모든 단계 완료!")

    return model, results

# 전체 파이프라인 실행
# model, results = full_pipeline()
```
...
def get_cosine_schedule_with_warmup(optimizer, num_warmup_steps,
    ...
    - Warmup: 처음 몇 step 동안 learning rate 점진적 증가
    - Cosine Annealing: 이후 cosine 함수로 감소
    """
