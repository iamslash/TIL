...
### 그럼 트리거 없으면 테이블과 차이가 없는가

**맞다.** MV 에서 트리거(자동 INSERT)를 빼면 그냥 테이블이다.

```
MV = 테이블 + 자동 INSERT 트리거
MV - 트리거 = 테이블
```

| | 일반 테이블 | Materialized View |
|---|---|---|
| 데이터 넣는 주체 | **개발자** (INSERT 직접 호출) | **ClickHouse** (원본 INSERT 시 자동) |
| INSERT 코드 필요 | O | X (MV 선언만 하면 끝) |
| 저장 구조 | MergeTree 등 | 동일 (내부에 같은 엔진 테이블) |
| SELECT 성능 | 동일 | 동일 |
| 스토리지 | 동일 | 동일 |

물리적으로는 **완전히 같은 것**이다. 유일한 차이는 "자동으로 채워주느냐, 수동으로 채우느냐" 뿐이다.
