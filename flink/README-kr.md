...
RocksDB 는 Facebook 이 개발한 고성능 임베디드 키-값 저장소이다. 파일 시스템이 아니라 라이브러리이다.

Flink 에서 RocksDB State Backend 를 사용하면:

- **높은 성능**: SSD 에 최적화된 읽기/쓰기
- **별도 서비스 불필요**: Flink 프로세스에 임베디드로 동작하여 배포가 간단
- **증분 체크포인팅**: 전체 상태가 아닌 변경분만 저장하여 I/O 감소
- **비동기 스냅샷**: 체크포인팅 중에도 상태 업데이트가 차단되지 않음
- **디스크 스필**: 메모리에 담을 수 없는 큰 상태도 디스크로 자동 확장

## Task Manager 간 데이터 공유

Redis 같은 외부 시스템 없이 Task Manager 간 데이터를 공유하려면 Flink 의 **Broadcast State 패턴** 을 사용한다. 설정값, 규칙, 참조 데이터 등을 모든 연산자 인스턴스에 읽기 전용으로 공유할 수 있다.
...
이 패턴을 사용하면 Redis 등 외부 컴포넌트 없이도 Flink 내부에서 효율적으로 데이터를 공유할 수 있다.
