...
다음은 camera 가 scene 을 렌더링할 때 특정 fragment 의 world space position 을 shadow map 의 texture space position 으로 변환 하는 함수를 구현한 것이다. directX 와 opengl 은 texture 의 uv 좌표의 원점위치가 다르다. 따라서 `UNITY_HALF_TEXEL_OFFSET` 를 이용해서 플래폼별로 구현을 다르게 해야 한다.
