...
정규화(Regularization)는 머신러닝 모델이 **과적합(overfitting)**되는 것을 막는 기법입니다.

## 과적합이란?

시험공부를 할 때를 생각해봅시다. 만약 여러분이 문제집의 문제와 답을 **통째로 암기**한다면, 문제집에서는 100점을 받을 수 있습니다. 하지만 실제 시험에서 조금만 다르게 출제되면 제대로 풀지 못하게 됩니다. 이것이 바로 **과적합**입니다.

머신러닝에서도 마찬가지입니다:
- 모델이 **훈련 데이터**만 너무 정확하게 외우면 → 100% 정확도
- 하지만 **새로운 데이터**에서는 → 성능이 떨어짐

정규화는 이런 문제를 해결하기 위해 모델에 "너무 복잡하게 외우지 마!"라고 제약을 거는 것입니다.

## 가중치(Weight)와 손실 함수(Loss Function)

먼저 기본 개념을 이해해봅시다:

**가중치(w)**는 모델이 각 특성(feature)을 얼마나 중요하게 생각하는지를 나타내는 숫자입니다.
- 예: 집값 예측 모델
  - w₁ = 면적의 중요도
  - w₂ = 방 개수의 중요도
  - w₃ = 역세권 여부의 중요도

**손실 함수(Loss)**는 모델의 예측이 실제 값과 얼마나 다른지를 나타냅니다:
...
과적합이 발생하면 가중치 값들이 **지나치게 커지거나 복잡해집니다**. 정규화는 이를 방지하기 위해 손실 함수에 **"가중치가 크면 벌점을 줘!"**라는 항을 추가합니다.
...
여기서:
- λ (람다): 정규화 강도 (0보다 큰 숫자)
- |w|: 가중치의 절댓값
```

**핵심 특징: 불필요한 것을 완전히 제거**

L1은 가중치의 **절댓값의 합**을 벌점으로 줍니다. 이렇게 하면:
- 중요하지 않은 가중치는 → **정확히 0**이 됨
- 중요한 가중치만 → 남게 됨

**비유:**
- 책가방에 교과서 10권이 있는데 너무 무거움
- L1 정규화: "필요 없는 책은 **아예 빼버려!**"
- 결과: 5권만 남기고 나머지는 0 (완전히 제거)

**장점:**
- 모델이 단순해짐 (특성 선택 기능)
- 어떤 특성이 중요한지 명확히 알 수 있음

**예시:**
```
정규화 전:
집값 = 100×면적 + 3×방개수 + 0.5×층수 + 0.01×건축연도 + ...

L1 정규화 후:
집값 = 100×면적 + 3×방개수
(층수, 건축연도 등은 가중치가 0이 되어 제거됨)
```
...
L2는 가중치의 **제곱의 합**을 벌점으로 줍니다. 이렇게 하면:
- 모든 가중치가 → **골고루 작아짐**
- 어느 것도 0이 되지는 않음 (매우 작아질 뿐)
- 특히 큰 가중치를 더 강하게 제약 (제곱이므로)

**비유:**
- 책가방에 교과서 10권이 있는데 너무 무거움
- L2 정규화: "모든 책을 **조금씩 더 얇은 버전으로** 바꿔!"
- 결과: 10권 모두 남아있지만 전부 가벼워짐
...
```
정규화 전:
집값 = 100×면적 + 3×방개수 + 50×층수 + 80×건축연도

L2 정규화 후:
집값 = 70×면적 + 2×방개수 + 5×층수 + 3×건축연도
(모든 가중치가 줄어들었고, 특히 큰 값들이 더 많이 줄어듦)
```
...
|------|-----------|-----------|
| 가중치 처리 | 일부를 **0으로** 만듦 | 모든 가중치를 **작게** 만듦 |
| 특성 선택 | ✅ 자동으로 중요한 특성만 선택 | ❌ 모든 특성 사용 |
| 모델 해석 | 쉬움 (0인 것은 무시하면 됨) | 어려움 (모두 고려해야 함) |
| 수치 안정성 | 보통 | 좋음 |
| 언제 사용? | 특성이 많고 대부분 불필요할 때 | 모든 특성이 어느정도 중요할 때 |

## Weight Decay (가중치 감쇠)

**Weight Decay는 L2 정규화의 다른 이름입니다.**

왜 "감쇠(decay)"라고 부를까요?

```
학습 과정에서:
새 가중치 = 이전 가중치 - 학습률 × 기울기 - λ × 이전 가중치
                                            ↑
                                     매번 가중치를 조금씩 줄임
```

마치 시간이 지나면서 물체가 **자연스럽게 감속**하는 것처럼, 가중치도 매번 조금씩 줄어듭니다. 이것이 "decay(감쇠)"라는 이름의 유래입니다.

## λ (람다)의 역할

λ는 정규화의 강도를 조절하는 **하이퍼파라미터**입니다:
...
λ가 작을 때 (예: 0.001):
    약한 정규화 → 모델이 복잡해질 수 있음

λ가 적당할 때 (예: 0.1):
    균형 잡힌 모델 → 이상적

λ가 클 때 (예: 100):
    강한 정규화 → 과소적합 위험 (너무 단순)
```

**비유:**
- λ = 속도 제한
- λ가 크면 = 느리게 가야 함 (안전하지만 너무 느림)
- λ가 작으면 = 빠르게 갈 수 있음 (빠르지만 위험)
- 적절한 λ = 안전하면서도 효율적인 속도

## 실전에서는?

1. **L1을 선택하는 경우:**
   - 특성이 수백, 수천 개인데 대부분 불필요할 것 같을 때
   - 모델을 해석하기 쉽게 만들고 싶을 때
   - 예: 의료 진단 (중요한 증상 몇 개만 선택)

2. **L2를 선택하는 경우:**
   - 모든 특성이 어느 정도 중요할 것 같을 때
   - 모델의 안정성이 중요할 때
   - 예: 이미지 인식 (모든 픽셀이 정보를 담고 있음)

3. **둘 다 사용 (Elastic Net):**
   - L1과 L2를 동시에 적용
   - 수식: `Loss = (예측 오차) + λ₁×Σ|wᵢ| + λ₂×Σwᵢ²`
   - 두 방법의 장점을 모두 활용
